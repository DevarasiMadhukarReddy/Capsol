...
export default class LoanQuickAction extends NavigationMixin(LightningElement) {
    ...
    clear() {
        ...
        this.noOfAdvisorsVal = null;
        this.legalEntityTypeVal = '';
        this.currentRegistrationVal = '';
    ...
    // madhu added CS-3348The characters included are alpha-numeric, full stops, commas, and space
    @api showError(val1,val2){
        var searchbName = this.template.querySelector(".bName"); 
        var searchcurrentFN = this.template.querySelector(".currentFN");
            
        if(val1 == 'businessLegalNameDBA'){
      ...
      //Madhu end for special characters  
    }
    removeUndef(event) {
        console.log('DBA1');
    ...
    handlebusinessLegalNameDBA(event) {
        //let temp = Object.assign({}, this.businessDetails);
        //temp.businessLegalNameDBA = event.target.value;
        //this.formData.businessLegalNameDBA = event.target.value;
       //Madhu added CS-3154 to prevent... / "" special characters    
      /* var regex = new RegExp("^$|[a-zA-Z0-9 ,.\n\r]+");
        ...
        const selectedbusinessLegalNameDBA = new CustomEvent("handletabusinessdetails", {
            detail: {
                'businessLegalNameDBA': event.target.value
            }
        });
    ...
    handlenoOfAdvisors(event) {
        // let temp = Object.assign({}, this.businessDetails);
        //temp.noOfAdvisors = event.target.value;


        // this.formData.noOfAdvisors = this.noOfAdvisorsVal;

        const selectednoOfAdvisors = new CustomEvent("handletabusinessdetails", {
            detail: {
                'noOfAdvisors': event.target.value
            }
        });
    ...
    handlelegalEntityType(event) {
        // let temp = Object.assign({}, this.businessDetails);
        //   temp.legalEntity = event.target.value;

      console.log('eventtragetvalue'+event.target.value);

        const selectedlegalEntityType = new CustomEvent("handletabusinessdetails", {
            detail: {
    ...
    handlecurrentRegistration(event) {
        //let temp = Object.assign({}, this.businessDetails);
        //temp.currentRegistration = event.target.value;



        const selectedCurrentRegistration = new CustomEvent("handletabusinessdetails", {
            detail: {
                'currentRegistration': event.target.value
            }
        });
    ...
    handlecurrentCustodian(event) {
        //let temp = Object.assign({}, this.businessDetails);
        // temp.currentCustodian = event.target.value;
        //event.preventDefault();
        //this.businessDetails.currentCustodian = event.target.value;

        const selectedCurrentCustodian = new CustomEvent("handletabusinessdetails", {
            detail: {
                'currentCustodian': event.target.value
            }
            // bubbles: true,
            // composed: true
    ...
    saveFormData(event) {
       ...
       // console.log('businessLegalNameDBA'+JSON.stringify(this.formData.businessLegalNameDBA))
      //  console.log('this.recordId----' + JSON.stringify(this.recordId));
        let finalArr;
       // console.log('this.formData@@' + JSON.stringify(this.formData));
       // let businessLegalNameDBA = JSON.stringify(this.formData.businessLegalNameDBA)
       // console.log('businessLegalNameDBA@@'+businessLegalNameDBA)

        finalArr = Object.assign({}, this.formData);
        // console.log("this.formData!!!" + this.formData); 
        // console.log("JSON.stringify(this.formData)!!!" + JSON.stringify(this.formData));               
        // console.log("JSON.stringify(finalArr)"+JSON.stringify(finalArr))
    ...
    saveLA() {
       ...
       // console.log('$$$$$$$$$$$$$$' + JSON.stringify(finalArr));
        console.log('@@@' + JSON.stringify(finalArr));
        console.log('$$'+JSON.stringify(finalArr.businessLegalNameDBA))
    ...
    connectedCallback() {
        ...
        this.MidPage = false;
        this.LastPage = false;
        console.log('businessDetails***123'+JSON.stringify(this.businessDetails));
